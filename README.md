...
Daily aggregator of Maine-located taildragger classified listings,
...
[Louisiana](https://github.com/taildraggers/louisiana) repos' approach, with
the state filter swapped to "ME".
...
- Each fetched page's listing table is parsed back into `(title, url,
  ...
  Location cell ends in `, ME` (case-insensitive), then de-duplicated by
...
- The result is rendered into `docs/index.html` titled **"Taildragger Ads
  in Maine"**, using the exact same table/card layout, dark-mode
...
4. GitHub will publish the page at `https://taildraggers.github.io/maine/`
...
```html
<iframe
  src="https://taildraggers.github.io/maine/"
  title="Taildragger Ads in Maine"
...
- This repo could be copied/adapted for other states by changing
  `STATE_ABBR` and `_MAINE_LOCATION_RE` in `main.py`, and updating
  `PAGE_TITLE`.
