"""Aggregates Maine-located listings from every taildraggers.com
...
ones whose location ends in ", ME".
...
Indiana/Iowa/Kansas/Kentucky/Louisiana repos' approach, with the state filter
swapped to "ME".
"""
...
PAGE_TITLE = "Taildragger Ads in Maine"
STATE_ABBR = "ME"
...
_MAINE_LOCATION_RE = re.compile(r",\s*ME\s*$", re.IGNORECASE)


def _is_maine(location: str) -> bool:
    return bool(_MAINE_LOCATION_RE.search(location.strip()))
...
def collect_listings() -> list[Listing]:
    ...
    for repo in SOURCE_SITES:
        ...
        maine_rows = [row for row in rows if _is_maine(row.location)]
        print(f"  [{repo}] {len(rows)} listings, {len(maine_rows)} in Maine")
        all_listings.extend(maine_rows)
...
def render_html(listings: list[Listing]) -> str:
    ...
    rows_html = "".join(rows) if rows else (
        ...
        "No Maine listings found in the latest run.</td></tr>"
    )
...
def main() -> None:
    ...
    print(f"[main] total unique Maine listings: {len(listings)}")
